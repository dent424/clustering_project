function draw(data) {
  ...
  //Section adding chapter selection boxes
  ...
  //List of variables in the data and their order
  ...
  //List of objects for presentations
  var description_objects =[
    ...
    {question:'grnsol',x_y_pos:[100,100], description:["But many are also not willing", "to accept cuts to their standard", "of living for the sake of the", "environment"], text_x_y_pos:[20,57], box_size:[190,430], callout:['3','4','5']},
    {question:'grntaxes',x_y_pos:[100,100], description:["And even more are unwilling", "to pay higher taxes to", "protect the environment"], text_x_y_pos:[20,57], box_size:[160,410], callout:['3','4','5']},
  ...
  //function updating narrative text in presentation at start
  function update_dialog_box() {
    //Gets object for current dialog box
    ...
    //if there is a presentation box
    if (typeof params !== "undefined" && params['text_x_y_pos'][0]!==0) {
      ...
      //places a button that allows you to restart or skip the presentation
      ...
      //Changes class of bars marked to be called out during presentation so that they will change color
  ...
  function update_chart(question) {
    ...
    var new_data = data.filter(function(d){
		...
		x.domain(nested
      .sort(function(a,b){ //This sorts the bars in decending order only for topprob1      
              if ( question === 'topprob1'){
                return b.values['percent'] - a.values['percent'];
              }
              else {
                return 0;
              }})
    ...
    group.select('rect')
  				 ...
  				 .attr('height', 0)
  				 .attr('y',height)
			     .each('end', function(){ //This makes sure that transitions are only vertical and not orizontal
				      d3.select(this)
				        .attr('x',function(d) {
    				 	     return x(d.key);
    		        })
  ...
  //creates question navigation buttons	
  ...
  //listens for click and updates question
  left.on("click", function(d) {
  	current_question-- //current question tracks which question is being diesplayed
    description_counter = [1000] //ensures that there is no display text
    d3.selectAll('.callout')
      .attr('class','standard');
